                            ...
                            <template v-if="column.key === 'operation'">
                                <a-button-group>
                                    <button @click="handleEdit(record)"
                                        class="bg-yellow-400 hover:bg-yellow-500 text-white font-bold py-2 px-4 mx-2 rounded-full">
                                        Edit
                                    </button>
...
const dataSource = ref(page.props.employees);
...
// Method untuk menangani aksi ketika tombol New Data diklik
const handleNewData = () => {
    // Lakukan tindakan yang diinginkan di sini, misalnya menavigasi ke halaman pembuatan data baru
    console.log('Button New Data clicked!');
};
...
const handleDelete = (record: any) => {
    router.delete(`/employee/${record.unique_id}`, {
